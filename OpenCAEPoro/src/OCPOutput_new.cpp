...
std::vector<double> serializeSumItem(const SumItem& item) {
    ...
    // Serialize the size of each string and the strings themselves
    serialized.push_back(static_cast<double>(item.Item.length()));
    for (char c : item.Item) serialized.push_back(static_cast<double>(c));

    serialized.push_back(static_cast<double>(item.Obj.length()));
    for (char c : item.Obj) serialized.push_back(static_cast<double>(c));

    serialized.push_back(static_cast<double>(item.Unit.length()));
    for (char c : item.Unit) serialized.push_back(static_cast<double>(c));

    serialized.push_back(static_cast<double>(item.Type.length()));
    for (char c : item.Type) serialized.push_back(static_cast<double>(c));

    // Serialize the vector size and its contents
    serialized.push_back(static_cast<double>(item.val.size()));
    serialized.insert(serialized.end(), item.val.begin(), item.val.end());
...
SumItem deserializeSumItem(const std::vector<double>& serialized, size_t& offset) {
    ...
    // Deserialize Item
    size_t length = static_cast<size_t>(serialized[offset++]);
    item.Item.resize(length);
    for (size_t i = 0; i < length; ++i) {
        item.Item[i] = static_cast<char>(serialized[offset++]);
    }

    // Deserialize Obj
    length = static_cast<size_t>(serialized[offset++]);
    item.Obj.resize(length);
    for (size_t i = 0; i < length; ++i) {
        item.Obj[i] = static_cast<char>(serialized[offset++]);
    }

    // Deserialize Unit
    length = static_cast<size_t>(serialized[offset++]);
    item.Unit.resize(length);
    for (size_t i = 0; i < length; ++i) {
        item.Unit[i] = static_cast<char>(serialized[offset++]);
    }

    // Deserialize Type
    length = static_cast<size_t>(serialized[offset++]);
    item.Type.resize(length);
    for (size_t i = 0; i < length; ++i) {
        item.Type[i] = static_cast<char>(serialized[offset++]);
    }

    // Deserialize val vector
    size_t vecSize = static_cast<size_t>(serialized[offset++]);
    item.val.resize(vecSize);
    for (size_t i = 0; i < vecSize; ++i) {
        item.val[i] = serialized[offset++];
    }
...
void gatherSumdata(const std::vector<SumItem>& Sumdata, int rank, int size) {
    // Step 1: Serialize Sumdata on every rank
    std::vector<double> serializedData;
    for (const auto& item : Sumdata) {
        std::vector<double> serItem = serializeSumItem(item);
        serializedData.insert(serializedData.end(), serItem.begin(), serItem.end());
    }

    // Step 2: Synchronize all processes to ensure serialization is complete
    MPI_Barrier(MPI_COMM_WORLD);

    // Get the size of serialized data on the current process
    int localSize = serializedData.size();

    // Step 3: Gather the sizes of serialized data from all ranks at rank 0
    std::vector<int> recvCounts(size);       // Will hold the sizes of data from all processes
    std::vector<int> displacements(size);    // Will hold the displacements for gathered data

    MPI_Gather(&localSize, 1, MPI_INT, recvCounts.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Only rank 0 needs to allocate space to receive all the data
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (i > 0) displacements[i] = displacements[i-1] + recvCounts[i-1];
        }
        gatheredData.resize(totalSize);
    }

    // Step 4: Gather the serialized data from all ranks at rank 0
    MPI_Gatherv(serializedData.data(), localSize, MPI_DOUBLE,
                gatheredData.data(), recvCounts.data(), displacements.data(),
                MPI_DOUBLE, 0, MPI_COMM_WORLD);

    // Step 5: Rank 0 processes the gathered data
    if (rank == 0) {
        size_t offset = 0;
        std::vector<SumItem> gatheredSumdata;
        while (offset < gatheredData.size()) {
            gatheredSumdata.push_back(deserializeSumItem(gatheredData, offset));
        }

        // Now gatheredSumdata contains all SumItems from all processes
        // Process gatheredSumdata as needed
    }
}
...
{
    string   FileOut;
        gatherSumdata(Sumdata, rank, 256);
